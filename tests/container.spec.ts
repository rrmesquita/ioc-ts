...
import { UseCase } from "./examples/use-case";
...
describe("Container", () => {
  ...
  describe.each([
    ...
    it(`should return the associated value of key: ${key.toString()}`, () => {
      const c = createContainer().bind(key, toValue(value));
  ...
  describe("toFunction()", () => {
    it("should return the associated function", () => {
      const c = createContainer().bind(
        DI.SIMPLE_FUNCTION,
        toFunction(sayHelloWorld),
      );

      const sayHello = c.get(DI.SIMPLE_FUNCTION);
  ...
  describe("toHigherOrderFunction()", () => {
    describe("When the higher order function has dependencies", () => {
      ...
      describe("When the dependencies are defined in an array", () => {
        it("should return the function with all its dependencies resolved", () => {
          // Arrange
          container.bind(
            DI.HIGHER_ORDER_FUNCTION_WITH_DEPENDENCIES,
    ...
    describe.each([
      ...
      "When the higher order function has no dependencies",
      ({ dependencies }) => {
        it("should just return the function", () => {
          // Arrange
          container.bind(DI.DEP1, toValue("dependency1"));
          container.bind(
            DI.HIGHER_ORDER_FUNCTION_WITHOUT_DEPENDENCIES,
            toHigherOrderFunction(
              HigherOrderFunctionWithoutDependency,
              dependencies,
            ),
          ...
          // Act
          const myService = container.get<ServiceWithoutDependencyInterface>(
            DI.HIGHER_ORDER_FUNCTION_WITHOUT_DEPENDENCIES,
          );
  ...
  describe("toCurry()", () => {
    describe("When the function has dependencies", () => {
      ...
      describe("When the dependencies are defined in an array", () => {
        it("should return the function with all its dependencies resolved", () => {
          // Arrange
          container.bind(
            DI.CURRIED_FUNCTION_WITH_DEPENDENCIES,
            toCurry(curriedFunctionWithDependencies, [DI.DEP1]),
          );

          // Act
          const myService = container.get<CurriedFunctionWithDependencies>(
            DI.CURRIED_FUNCTION_WITH_DEPENDENCIES,
          );
      ...
      describe("When the dependencies are defined in an object", () => {
        it("should return the function with all its dependencies resolved", () => {
          // Arrange
          container.bind(
            DI.CURRIED_FUNCTION_WITH_DEPENDENCIES_OBJECT,
          ...
          // Assert
          expect(myService("curry")).toBe(
            "Hello curry with dependency1 and 42",
          );
        });
      });
    ...
    describe.each([
      ...
      it("should just return the function", () => {
        ...
        container.bind(DI.DEP1, toValue("dependency1"));
        container.bind(
          DI.CURRIED_FUNCTION_WITHOUT_DEPENDENCIES,
          toCurry(curriedFunctionWithoutDependencies, dependencies),
        );

        // Act
        const myService = container.get<CurriedFunctionWithoutDependencies>(
          DI.CURRIED_FUNCTION_WITHOUT_DEPENDENCIES,
        );
      ...
      container.bind(
        ...
        toFactory(() => {
          return HigherOrderFunctionWithDependencyObject({
            dep1: container.get(DI.DEP1),
            dep2: container.get(DI.DEP2),
          });
        }),
      );
    ...
    describe("When the class has dependencies", () => {
      describe("When the dependencies are defined in an array", () => {
        it("should return the instance with the resolved dependencies", () => {
          ...
          container.bind(DI.DEP2, toValue(42));
          container.bind(
            DI.CLASS_WITH_DEPENDENCIES,
            toClass(MyServiceClass, [DI.DEP1, DI.DEP2]),
          );

          // Act
          const myService = container.get<MyServiceClassInterface>(
            DI.CLASS_WITH_DEPENDENCIES,
          );
      ...
      describe("When the dependencies are defined in an object", () => {
        it("should return the instance with the resolved dependencies", () => {
          ...
          container.bind(
            DI.CLASS_WITH_DEPENDENCIES,
            toClass(MyServiceClassWithDependencyObject, {
              dep1: DI.DEP1,
              dep2: DI.DEP2,
            }),
          );
  ...
  describe("When a dependency is missing", () => {
    it("should throw an error", () => {
      // Act
      const expectCall = expect(() =>
        container.get<string>(DI.NOT_REGISTERED_VALUE),
      );

      // Assert
      expectCall.toThrowError(
        `No binding found for key: ${DI.NOT_REGISTERED_VALUE.toString()}`,
      );
    });
  });
  ...
  describe("When a circular dependency is detected", () => {
    it("should throw an error", () => {
      ...
      // Assert
      expectCall.toThrowError(
        /Circular dependency detected: Symbol\(CIRCULAR_A\) -> Symbol\(CIRCULAR_B\) -> Symbol\(CIRCULAR_A\)/,
      );
    });
  });
});
