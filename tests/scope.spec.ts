...
describe("Scope", () => {
  ...
  describe("Factories", () => {
    describe.each([{ scope: undefined }, { scope: "singleton" }])(
      'When the scope is default or defined to "singleton"',
      ({ scope }) => {
    ...
    describe.each([{ scope: undefined }, { scope: "singleton" }])(
      ...
      ({ scope }) => {
        it("should return the same instance", () => {
          ...
          container.bind(
            DI.MY_SERVICE,
            toClass(MyServiceClass, [DI.DEP1, DI.DEP2], scope as Scope),
          );
    ...
    describe.each([{ scope: undefined }, { scope: "singleton" }])(
      ...
      ({ scope }) => {
        it("should return the same instance", () => {
          ...
          const myService1 = container.get<CurriedFunctionWithDependencies>(
            DI.MY_SERVICE,
          );

          // Act
          const myService2 = container.get<CurriedFunctionWithDependencies>(
            DI.MY_SERVICE,
          );
  ...
  describe("When a scoped dependency is resolved outside of a scope", () => {
    it("should throw an error", () => {
      ...
      // Act & Assert
      expect(() =>
        container.get<MyServiceInterface>(DI.MY_SERVICE),
      ).toThrowError(
        `Cannot resolve scoped binding outside of a scope: ${DI.MY_SERVICE.toString()}`,
      );
    });
  });
  ...
  describe("When an unknown scope is used during binding", () => {
    it("should throw an error", () => {
      ...
      container.bind(
        DI.MY_SERVICE,
        toClass(MyServiceClass, [], "unknown" as any),
      );

      // Act & Assert
      expect(() =>
        container.get<MyServiceClassInterface>(DI.MY_SERVICE),
